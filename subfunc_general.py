...
def inter_out_left(vertex, f, a, b, i, n, jd, jv):
    A = np.zeros((4, 4 * (n - 1) ** 2))
    B = np.zeros((4))
    mu = np.sqrt(b[i] / a[i])
    mu_l = np.sqrt(b[i - (n - 1)] / a[i - (n - 1)])
...
def inter_out_up(vertex, f, a, b, i, n, jd, jv):
    A = np.zeros((4, 4 * (n - 1) ** 2))
    B = np.zeros((4))
    mu = np.sqrt(b[i] / a[i])
    mu_l = np.sqrt(b[i - (n - 1)] / a[i - (n - 1)])
...
def inter_out_right(vertex, f, a, b, i, n, jd, jv):
    A = np.zeros((4, 4 * (n - 1) ** 2))
    B = np.zeros((4))
    mu = np.sqrt(b[i] / a[i])
    mu_l = np.sqrt(b[i - (n - 1)] / a[i - (n - 1)])
...
def inter_out_down(vertex, f, a, b, i, n, jd, jv):
    A = np.zeros((4, 4 * (n - 1) ** 2))
    B = np.zeros((4))
    mu = np.sqrt(b[i] / a[i])
    mu_l = np.sqrt(b[i - (n - 1)] / a[i - (n - 1)])
...
def inter_in_left(vertex, f, a, b, i, n, jd, jv):
    A = np.zeros((4, 4 * (n - 1) ** 2))
    B = np.zeros((4))
    mu = np.sqrt(b[i] / a[i])
    mu_l = np.sqrt(b[i - (n - 1)] / a[i - (n - 1)])
...
def inter_in_up(vertex, f, a, b, i, n, jd, jv):
    A = np.zeros((4, 4 * (n - 1) ** 2))
    B = np.zeros((4))
    mu = np.sqrt(b[i] / a[i])
    mu_l = np.sqrt(b[i - (n - 1)] / a[i - (n - 1)])
...
def inter_in_right(vertex, f, a, b, i, n, jd, jv):
    A = np.zeros((4, 4 * (n - 1) ** 2))
    B = np.zeros((4))
    mu = np.sqrt(b[i] / a[i])
    mu_l = np.sqrt(b[i - (n - 1)] / a[i - (n - 1)])
...
def inter_in_down(vertex, f, a, b, i, n, jd, jv):
    A = np.zeros((4, 4 * (n - 1) ** 2))
    B = np.zeros((4))
    mu = np.sqrt(b[i] / a[i])
    mu_l = np.sqrt(b[i - (n - 1)] / a[i - (n - 1)])
...
def inter_top(vertex, f, a, b, i, n, jd, jv, interface):
    h = 1 / (n - 1)
