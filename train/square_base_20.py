...
def bilinear_interp_20_to_100(input_tensor):
    """
    从20×20到100×100的双线性插值
    参数:
    input_tensor: 形状为 [batch, 20, 20] 的张量

    返回:
    output: 形状为 [batch, 100, 100] 的插值结果
    """
    # 添加channel维度 -> 插值 -> 移除channel维度
    return F.interpolate(
        input_tensor.unsqueeze(1),  # [batch, 1, 20, 20]
        size=(100, 100),
        mode='bilinear',
        align_corners=True
    ).squeeze(1)  # [batch, 100, 100]
...
def main():
    ...
    x_train = input[:ntrain].reshape(ntrain, 20, 20)
    y_train = output[:ntrain].reshape(ntrain, s1, s2)
    x_test = input[-ntest:].reshape(ntest, 20, 20)
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            ...
            u = bilinear_interp_20_to_100(out.squeeze())
