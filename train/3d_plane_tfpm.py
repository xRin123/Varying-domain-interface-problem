...
def batch_upsample_repeat_21_to_41(u_batch):
    """
    通过重复每个值的方法上采样（批量版本）

    参数:
    u_batch: 形状为(batch, 21, 21, 21)的数组或张量

    返回:
    u_batch_41: 形状为(batch, 41, 41, 41)的上采样结果
    """
    batch_size = u_batch.shape[0]

        # 方法1: 使用repeat_interleave
    u_batch_41 = u_batch.repeat_interleave(2, dim=1)  # 在第一个空间维度重复
    u_batch_41 = u_batch_41.repeat_interleave(2, dim=2)  # 在第二个空间维度重复
    u_batch_41 = u_batch_41.repeat_interleave(2, dim=3)  # 在第三个空间维度重复

        # 截断到41×41×41
    u_batch_41 = u_batch_41[:, :41, :41, :41]

    return u_batch_41

def batch_upsample_repeat_11_to_41(u_batch):

    batch_size = u_batch.shape[0]

    # 创建索引网格
    idx = torch.arange(41, device=u_batch.device) // 4
    idx = torch.clamp(idx, 0, 10)

    # 创建索引张量
    idx_i = idx.view(-1, 1, 1).expand(-1, 41, 41)
    idx_j = idx.view(1, -1, 1).expand(41, -1, 41)
    idx_k = idx.view(1, 1, -1).expand(41, 41, -1)

    # 批量索引
    u_batch_41 = u_batch[:, idx_i, idx_j, idx_k]
...
def main():
    ...
    PATH_XY = path + '/area_sdf.npy'
    PATH_EX = path + '/ex.npy'
    PATH_EY = path + '/ey.npy'
    PATH_EZ = path + '/ez.npy'
    PATH_EXNE = path + '/exne.npy'
    PATH_EYNE = path + '/eyne.npy'
    PATH_EZNE = path + '/ezne.npy'
    PATH_F = path + '/f.npy'
    ...
    ex = torch.tensor(INPUT_EX, dtype=torch.float).cuda()
    ey = torch.tensor(INPUT_EY, dtype=torch.float).cuda()
    ez = torch.tensor(INPUT_EZ, dtype=torch.float).cuda()
    exne = torch.tensor(INPUT_EXNE, dtype=torch.float).cuda()
    eyne = torch.tensor(INPUT_EYNE, dtype=torch.float).cuda()
    ezne = torch.tensor(INPUT_EZNE, dtype=torch.float).cuda()
    f = torch.tensor(INPUT_F, dtype=torch.float).cuda().reshape(1, 41, 41, 41)
    f = f.repeat(batch_size, 1, 1, 1)
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            ...
            out41 = batch_upsample_repeat_21_to_41(out)

            u = f + out41[..., 0] * ex + out41[..., 1] * exne + out41[..., 2] * ey + out41[..., 3] * eyne + out41[..., 4] * ez + out41[..., 5] * ezne
        ...
        with torch.no_grad():
            for x, y in test_loader:
                ...
                u = f + out41[..., 0] * ex + out41[..., 1] * exne + out41[..., 2] * ey + out41[..., 3] * eyne + out41[
                    ..., 4] * ez + out41[..., 5] * ezne
