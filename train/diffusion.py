...
class FNO2d(nn.Module):
    ...
    def forward(self, x, f):
        ...
        x = torch.cat((torch.unsqueeze(x,-1), torch.unsqueeze(f,-1), grid), dim=-1)
        x = self.fc0(x)
        x = x.permute(0, 3, 1, 2)
        ...
        x2 = self.w0(x)
        x = x1 + x2
        x = F.gelu(x) #(batch, 64, 110, 110) >> # (batch, 65, 110, 110)
...
def main():
    ...
    PATH_Sigma = path + '/Y_data_t01.npy'
    PATH_XY = path + '/area_sdf.npy'
    PATH_F = path + '/X_data_t0.npy'
    ...
    x_train = input[:ntrain].reshape(ntrain, s1, s2)
    y_train = output[:ntrain].reshape(ntrain, s1, s2)
    x_test = input[ntrain:Ntotal].reshape(ntest, s1, s2)
    y_test = output[-ntest:].reshape(ntest, s1, s2)
    f_train = f[:ntrain].reshape(ntrain, s1, s2)
    f_test = f[-ntest:].reshape(ntest, s1, s2)

    train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(x_train, y_train, f_train), batch_size=batch_size,
                                               shuffle=True)
    test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(x_test, y_test, f_test), batch_size=batch_size,
                                              shuffle=False)
