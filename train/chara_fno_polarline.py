...
class FNO2d(nn.Module):
    ...
    def forward(self, x, f, j):
        ...
        x = torch.cat((torch.unsqueeze(x,-1), torch.unsqueeze(f,-1), torch.unsqueeze(j,-1), grid), dim=-1)
        x = self.fc0(x)
        x = x.permute(0, 3, 1, 2)
...
################################################################
def main():
    path = '../data/polar_line'
    #path = 'data/chara_fno'#'../data/polar_line/chara_fno'

    PATH_Sigma = path + '/solve_uniform.npy'
    PATH_XY = path + '/area_chara.npy'
    PATH_F = path + '/chara_f.npy'
    PATH_J = path + '/chara_j.npy'
    ...
    x_test = input[-ntest:].reshape(ntest, s1, s2)
