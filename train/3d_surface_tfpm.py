...
class FNO3d(nn.Module):
    def __init__(self, modes1, modes2, modes3, width, padding=9):
        ...
        self.fc0 = nn.Linear(5, self.width)  # input channel: (a(x,y,z), j, x, y, z)
    ...
    def forward(self, x, jv):
        ...
        # 连接输入: (a, j, x, y, z)
        x = torch.cat((torch.unsqueeze(x, -1), grid, torch.unsqueeze(jv, -1)), dim=-1)
...
def batch_upsample_repeat_26_to_51(u_batch):

    batch_size = u_batch.shape[0]

        # 方法1: 使用repeat_interleave
    u_batch_51 = u_batch.repeat_interleave(2, dim=1)  # 在第一个空间维度重复
    u_batch_51 = u_batch_51.repeat_interleave(2, dim=2)  # 在第二个空间维度重复
    u_batch_51 = u_batch_51.repeat_interleave(2, dim=3)  # 在第三个空间维度重复

        # 截断到41×41×41
    u_batch_51 = u_batch_51[:, :51, :51, :51]
...
def main():
    path = 'data/surface'
    #path = '../data/3d_surface'

    PATH_Sigma = path + '/solve51.npy'
    PATH_XY = path + '/sdf26.npy'
    PATH_JV = path + '/inter26.npy'
    PATH_EX = path + '/ex_51.npy'
    PATH_EY = path + '/ey_51.npy'
    PATH_EZ = path + '/ez_51.npy'
    PATH_EXNE = path + '/exne_51.npy'
    PATH_EYNE = path + '/eyne_51.npy'
    PATH_EZNE = path + '/ezne_51.npy'

    INPUT_XY = np.load(PATH_XY)
    INPUT_JV = np.load(PATH_JV)
    OUTPUT_Sigma = np.load(PATH_Sigma)
    INPUT_EX = np.load(PATH_EX)
    INPUT_EY = np.load(PATH_EY)
    INPUT_EZ = np.load(PATH_EZ)
    INPUT_EXNE = np.load(PATH_EXNE)
    INPUT_EYNE = np.load(PATH_EYNE)
    INPUT_EZNE = np.load(PATH_EZNE)
    ...
    jv = torch.tensor(INPUT_JV, dtype=torch.float)
    output = torch.tensor(OUTPUT_Sigma, dtype=torch.float)
    x_train = input[:ntrain].reshape(ntrain, s1, s2, s3)
    v_train = jv[:ntrain].reshape(ntrain, s1, s2, s3)
    y_train = output[:ntrain].reshape(ntrain, 51, 51, 51)
    x_test = input[-ntest:].reshape(ntest, s1, s2, s3)
    v_test = jv[-ntest:].reshape(ntest, s1, s2, s3)
    y_test = output[-ntest:].reshape(ntest, 51, 51, 51)
    ex = torch.tensor(INPUT_EX, dtype=torch.float).cuda().view(51,51,51)
    ey = torch.tensor(INPUT_EY, dtype=torch.float).cuda().view(51,51,51)
    ez = torch.tensor(INPUT_EZ, dtype=torch.float).cuda().view(51,51,51)
    exne = torch.tensor(INPUT_EXNE, dtype=torch.float).cuda().view(51,51,51)
    eyne = torch.tensor(INPUT_EYNE, dtype=torch.float).cuda().view(51,51,51)
    ezne = torch.tensor(INPUT_EZNE, dtype=torch.float).cuda().view(51,51,51)


    train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(x_train, v_train, y_train), batch_size=batch_size,
                                               shuffle=True)
    test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(x_test, v_test, y_test), batch_size=batch_size,
                                              shuffle=False)
    ...
    f = torch.ones(batch_size, 51, 51, 51).cuda()
    ...
    for ep in range(epochs):
        ...
        for x, v, y in train_loader:
            x, v, y = x.cuda(), v.cuda(), y.cuda()

            optimizer.zero_grad()
            out = model(x, v)

            out51 = batch_upsample_repeat_26_to_51(out)

            u = f + out51[..., 0] * ex + out51[..., 1] * exne + out51[..., 2] * ey + out51[..., 3] * eyne + out51[
                ..., 4] * ez + out51[..., 5] * ezne
        ...
        with torch.no_grad():
            for x, v, y in test_loader:
                ...
                test_l2 += myloss(u.view(batch_size,-1),y.view(batch_size,-1)).item()
